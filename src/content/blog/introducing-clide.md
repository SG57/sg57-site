...
Every session includes a built-in scratchpad (Monaco Editor) for notes alongside the terminal, session tagging with NATO alphabet naming, collection-based batch spawning for quick project setup, and sidecar process management for companion tasks. The entire interface is keyboard-first with a command palette (Ctrl+Shift+P), and every action is discoverable through it. Clide is free to use and built for Windows. Download it at [sg57.dev/clide](/clide).
